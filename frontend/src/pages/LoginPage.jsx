...
function LoginPage() {
  const { login, pendingGstin } = useDashboard();
  const navigate = useNavigate();
  const [gstin, setGstin] = useState(pendingGstin || '');
  const [otp, setOtp] = useState('');

  // Update gstin if pendingGstin changes (e.g. user goes back and searches again)
  useEffect(() => {
    if (pendingGstin) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden bg-white">
      ...
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-white to-blue-50 items-center justify-center relative p-12 overflow-hidden">
        ...
        <div className="absolute top-[-20%] left-[-20%] w-[500px] h-[500px] bg-[#2D5BFF]/10 rounded-full blur-[100px] animate-pulse"></div>

        <div className="max-w-md text-center space-y-8 animate-in slide-in-from-left duration-1000">
          <div className="flex items-center justify-center mb-6">
            <div className="w-16 h-16 rounded-2xl bg-[#2D5BFF] flex items-center justify-center shadow-2xl shadow-[#2D5BFF]/30">
              <Zap className="text-white fill-current" size={32} />
            </div>
          </div>

          <h1 className="text-4xl font-black text-[#2D5BFF] leading-tight">
          ...
          <div className="pt-12 grid grid-cols-2 gap-4 text-left">
            ...
            <div className="p-4 bg-white rounded-2xl shadow-sm border border-blue-100">
              <Fingerprint className="text-[#2D5BFF] mb-2" size={20} />
      ...
      <div className="flex-1 md:w-1/2 bg-[#f8faff] flex items-center justify-center p-6 relative">
        ...
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#2D5BFF]/5 rounded-full blur-3xl"></div>
        ...
        <div className="w-full max-w-sm animate-in slide-in-from-bottom duration-700 [animation-delay:200ms]">
          <div className="bg-white/60 backdrop-blur-md border border-white/50 rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] p-10">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                ...
                <div className="relative group">
                  <Zap className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#2D5BFF] transition-colors" size={18} />
                  <input
                    type="text"
                    placeholder="27AABCU1234F1Z5"
                    value={gstin}
                    onChange={(e) => setGstin(e.target.value.toUpperCase())}
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#2D5BFF]/10 focus:border-[#2D5BFF] outline-none transition-all font-bold text-slate-700 uppercase"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  <Smartphone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#2D5BFF] transition-colors" size={18} />
                  <input
                    type="text"
                    maxLength={6}
                    placeholder="······"
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#2D5BFF]/10 focus:border-[#2D5BFF] outline-none transition-all font-bold text-slate-700 tracking-[0.5em] placeholder:tracking-normal"
                    required
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 px-1">
                <input type="checkbox" id="consent" className="rounded-md border-slate-300 text-[#2D5BFF] focus:ring-[#2D5BFF]" required />
              ...
              <button
                type="submit"
                className="w-full py-5 bg-[#2D5BFF] text-white font-bold rounded-2xl shadow-xl shadow-[#2D5BFF]/30 hover:bg-[#2D5BFF]/90 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300"
              >
                Secure Login
              </button>
            </form>
            ...
            <div className="mt-8 pt-8 border-t border-slate-100 flex items-center justify-between">
              ...
              <a href="#" className="text-[10px] font-bold text-[#2D5BFF] flex items-center gap-1 hover:underline">
          ...
          <div className="mt-8 text-center">
            <p className="text-xs text-slate-400 font-medium">New MSME? <span className="text-[#2D5BFF] font-bold cursor-pointer hover:underline">Register Profile</span></p>
          </div>
        </div>
      </div>
    </div>
